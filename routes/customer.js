...
router.post('/createCustomer', async (req, res) => {
  ...
  let promiseArray = req.body.idProofs.length ? [getLatLongDetails(customerdetails), uploadImage(req)] : [getLatLongDetails(customerdetails)]
...
router.post('/updateCustomer', async (req, res) => {
  ...
  // let customerDetailsQuery = "insert  into customerdetails (customerName,mobileNumber,EmailId,Address1,gstNo,registeredDate,invoicetype,natureOfBussiness,creditPeriodInDays,referredBy,isActive,qrcodeId,latitude,longitude,customerType,organizationName,createdBy) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  let promiseArray = req.body.idProofs.length ? [getLatLongDetails(customerdetails), customer_id_proof ? updateProofs(req) : uploadImage(req)] : [getLatLongDetails(customerdetails)]
  Promise.all(promiseArray)
    .then(response => {
      let customerIdProof = req.body.idProofs.length && customer_id_proof ? customer_id_proof : response[1]
