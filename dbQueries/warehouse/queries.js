...
warehouseQueries.getDeliverysByCustomerOrderId = (customerOrderId, callback) => {
    let query = "select c.customerOrderId,c.contactPerson,c.deliveryLocation,c.creationType,CASE WHEN c.creationType='manual' THEN c.EmailId ELSE cd.EmailId  END AS EmailId,c.existingCustomerId,c.distributorId,c.customerType,c.customerName,c.phoneNumber,c.address,c.routeId,c.driverId,c.isDelivered,c.dcNo,c.20LCans AS product20L,c.1LBoxes AS product1L,c.500MLBoxes AS product500ML,c.300MLBoxes AS product300ML,c.2LBoxes AS product2L,c.price20L, c.price2L, c.price1L, c.price500ML, c.price300ML,r.*,c.vehicleId,v.vehicleName,d.driverName,d.mobileNumber FROM customerorderdetails c LEFT JOIN routes r  ON c.routeId=r.routeid left JOIN driverdetails d ON c.driverId=d.driverid LEFT JOIN VehicleDetails v ON c.vehicleId=v.vehicleId left JOIN customerdetails cd ON c.existingCustomerId=cd.customerId  WHERE customerOrderId=" + customerOrderId;
    return executeGetQuery(query, callback)
}
...
warehouseQueries.getReturnedEmptyCans = async ({ departmentId, date }, callback) => {
