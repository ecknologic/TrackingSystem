...
warehouseQueries.getAllDcDetails = (input, callback) => {
    ...
    if (customerIds && fromDate && toDate && departmentId) {
        query = "select c.returnEmptyCans,c.customerOrderId,c.deliveredDate,c.customerName,c.phoneNumber,c.address,c.routeId,c.driverId,c.isDelivered,c.dcNo,c.20LCans AS product20L,c.1LBoxes AS product1L,c.500MLBoxes AS product500ML,c.300MLBoxes AS product300ML,c.2LBoxes AS product2L,r.*,d.driverName,d.mobileNumber FROM customerorderdetails c LEFT JOIN routes r  ON c.routeId=r.routeid left JOIN driverdetails d ON c.driverId=d.driverid  WHERE existingCustomerId IN (?) AND (DATE(deliveryDate) BETWEEN ? AND ?) AND warehouseId=? AND isDelivered='Completed' ORDER BY c.dcNo DESC";
        options = [customerIds, fromDate, toDate, departmentId]
    } else if (customerIds && fromDate && toDate && !departmentId) {
        query = "select c.returnEmptyCans,c.customerOrderId,c.deliveredDate,c.customerName,c.phoneNumber,c.address,c.routeId,c.driverId,c.isDelivered,c.dcNo,c.20LCans AS product20L,c.1LBoxes AS product1L,c.500MLBoxes AS product500ML,c.300MLBoxes AS product300ML,c.2LBoxes AS product2L,r.*,d.driverName,d.mobileNumber FROM customerorderdetails c LEFT JOIN routes r  ON c.routeId=r.routeid left JOIN driverdetails d ON c.driverId=d.driverid  WHERE existingCustomerId IN (?) AND (DATE(deliveryDate) BETWEEN ? AND ?) AND isDelivered='Completed' ORDER BY c.dcNo DESC";
        options = [customerIds, fromDate, toDate]
    }
    else if (customerIds) {
        query = "select c.returnEmptyCans,c.customerOrderId,c.deliveredDate,c.customerName,c.phoneNumber,c.address,c.routeId,c.driverId,c.isDelivered,c.dcNo,c.20LCans AS product20L,c.1LBoxes AS product1L,c.500MLBoxes AS product500ML,c.300MLBoxes AS product300ML,c.2LBoxes AS product2L,r.*,d.driverName,d.mobileNumber FROM customerorderdetails c LEFT JOIN routes r  ON c.routeId=r.routeid left JOIN driverdetails d ON c.driverId=d.driverid  WHERE existingCustomerId IN (?) AND warehouseId=? AND isDelivered='Completed' ORDER BY c.dcNo DESC";
...
warehouseQueries.getTotalSales = (input, callback) => {
    ...
    let query = "select SUM(c.20LCans) AS product20LCount,SUM(c.1LBoxes) AS product1LCount,SUM(c.500MLBoxes) AS product500MLCount,SUM(c.300MLBoxes) AS product300MLCount,SUM(c.2LBoxes) AS product2LCount FROM customerorderdetails c WHERE DATE(`deliveredDate`) <= ? AND isDelivered='Completed'";
    let options = [endDate]

    if (fromStart !== 'true') {
        query = "select SUM(c.20LCans) AS product20LCount,SUM(c.1LBoxes) AS product1LCount,SUM(c.500MLBoxes) AS product500MLCount,SUM(c.300MLBoxes) AS product300MLCount,SUM(c.2LBoxes) AS product2LCount FROM customerorderdetails c WHERE DATE(`deliveredDate`) >= ? AND DATE(`deliveredDate`) <= ? AND isDelivered='Completed'";
        options = [startDate, endDate]
    }

    if (departmentId != 'All') {
        query = "select SUM(c.20LCans) AS product20LCount,SUM(c.1LBoxes) AS product1LCount,SUM(c.500MLBoxes) AS product500MLCount,SUM(c.300MLBoxes) AS product300MLCount,SUM(c.2LBoxes) AS product2LCount FROM customerorderdetails c WHERE  DATE(`deliveredDate`) <= ? AND warehouseId=? AND isDelivered='Completed'";
...
warehouseQueries.saveWarehouseStockDetails = (input, callback) => {
    ...
    let query = "insert into warehousestockdetails (warehouseId,20LCans,1LBoxes,500MLBoxes,300MLBoxes,2LBoxes,damaged20LCans,damaged1LBoxes,damaged500MLBoxes,damaged300MLBoxes,damaged2LBoxes,deliveryDate,isConfirmed,DCNO,damagedDesc,hasDamaged) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    let requestBody = [departmentId, product20L, product1L, product500ML, product300ML, product2L, damaged20LCans, damaged1LBoxes, damaged500MLBoxes, damaged300MLBoxes, damaged2LBoxes, deliveryDate, '1', dcNo, damagedDesc, '0']

    if (isDamaged) requestBody = [departmentId, product20L - damaged20LCans, product1L - damaged1LBoxes, product500ML - damaged500MLBoxes, product300ML - damaged300MLBoxes, product2L - damaged2LBoxes, damaged20LCans, damaged1LBoxes, damaged500MLBoxes, damaged300MLBoxes, damaged2LBoxes, deliveryDate, '1', dcNo, damagedDesc, '1']
