...
customerQueries.generatePDF = (input, callback) => {
    ...
    let query = "SELECT c.gstNo,c.customerId,c.createdBy,c.EmailId,c.customerName,c.organizationName,c.address1,d.address,c.gstNo,c.panNo,c.mobileNumber,SUM(co.20LCans),SUM(co.price20L),SUM(co.1LBoxes),SUM(co.price1L), SUM(co.500MLBoxes),SUM(co.price500ML),SUM(co.300MLBoxes),SUM(co.price300ML),SUM(co.2LBoxes),SUM(co.price2L) FROM customerdetails c INNER JOIN  customerorderdetails co ON c.customerId=co.existingCustomerId INNER JOIN DeliveryDetails d ON d.customer_Id=c.customerId  WHERE co.isDelivered='Completed' AND( DATE(co.deliveryDate) BETWEEN ? AND ?)"
